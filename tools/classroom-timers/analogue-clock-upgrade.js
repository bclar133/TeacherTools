(() => {
  ...
  if (window.__analogueClockUpgradeV1) return;
  window.__analogueClockUpgradeV1 = true;
  ...
  style.id = 'analogueClockUpgradeStyleV1';
  style.textContent = `
    ...
    #analogueClock .clock-face{
      ...
      background:
        radial-gradient(circle at 50% 50%,#fff 0 58%,#f3f5f5 76%,#dce3e6 100%)!important;
      box-shadow:
        0 22px 50px rgba(0,0,0,.35),
        inset 0 0 22px rgba(0,0,0,.14),
        inset 0 0 0 3px #9aa8b1!important;
      color:#21313d;
    }
    ...
    #analogueClock .clock-tick.major{
      width:4px;
      height:18px;
      margin-left:-2px;
      margin-top:-9px;
    ...
    #analogueClock .clock-hand.hour{
      width:12px!important;
      height:27%!important;
      background:#26333c!important;
    }
    #analogueClock .clock-hand.minute{
      width:8px!important;
      height:38%!important;
      background:#3b4d5a!important;
    }
    #analogueClock .clock-hand.second{
      width:3px!important;
      height:42%!important;
    ...
    html[data-theme="dark"] #clockStage{
      background:linear-gradient(145deg,#101c27,#071019)!important;
    }
    html[data-theme="dark"] #analogueClock .clock-face{
      border-color:#788792!important;
      background:radial-gradient(circle,#27333d 0 62%,#1d2730 78%,#121a21 100%)!important;
      color:#eef5f8!important;
      box-shadow:0 22px 50px rgba(0,0,0,.55),inset 0 0 24px rgba(0,0,0,.42),inset 0 0 0 3px #485761!important;
    }
  ...
  function show(styleName) {
    ...
    Object.entries(views).forEach(([key, view]) => {
      ...
      view.style.display = active ? 'grid' : 'none';
    });
